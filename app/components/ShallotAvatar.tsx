...
export default function ShallotAvatar({ airTemp, airHumidity, soilHumidity, soilTemp }: ShallotAvatarProps) {
  ...
  const bulbColor = soilCritical 
    ? '#502030' // Darker purple-red when critical
    : soilDry 
    ? '#703050' // Medium purple-red when dry
    : tempTooHigh
    ? '#904070' // Lighter purple-red when hot
    : '#9C669C'; // More purple-red when healthy (typical shallot color)

  const bulbShade = soilCritical 
    ? '#301020' // Darkest shade when critical
    : soilDry 
    ? '#502040' // Dark purple-red shade
    : tempTooHigh
    ? '#703060' // Medium shade when hot
    : '#7A4A7A'; // Deep purple-red shade (healthy shallot)
      ...
      >
        ...
        <defs>
          <radialGradient id="shallotGradient" cx="50%" cy="50%" r="50%">
            <stop offset="0%" stopColor="#E0B0D8" stopOpacity="0.3" />
            <stop offset="50%" stopColor="#A05080" stopOpacity="0.5" />
            <stop offset="100%" stopColor="#703050" stopOpacity="0.7" />
          </radialGradient>
        </defs>
