...
export default function ManualControl() {
  ...
  const sendPumpCommand = async (command: 'START' | 'STOP') => {
    ...
    try {
      // Insert command into database (trigger will handle MQTT)
      ...
      alert(`Pump ${command} command sent successfully!`)
    } catch (err: any) {
      ...
      {lastCommand && (
        <div className="mt-6 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
          ...
          <div className="text-sm text-blue-700 dark:text-blue-400 space-y-1">
            ...
            <p><strong>Issued:</strong> {new Date(lastCommand.issued_at).toLocaleString()}</p>
          </div>
        </div>
      )}
